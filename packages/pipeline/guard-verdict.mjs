...
export function renderGuardSummary(v = {}) {
  ...
  if (v.decision === "page") {
    ...
    // Redacted before truncation and defanging, for the same reason the fence
    // exists: everything in `detail` came from somewhere less trusted than this
    // renderer. A fence stops crafted markdown from rendering as structure; it
    // does nothing about a credential quoted inside the text.
    if (v.detail) {
